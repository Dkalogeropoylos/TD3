...
class Experiment:
    def __init__(self, environment, agent=None, load_models=False, config=None, participant_name=None,second_agent=None):
        ...
        if self.config['Experiment']['mode'] != 'human':
            ...
            #if self.config['TD3']['load_checkpoint'] == True:
                #self.agent.load_models()
            #if self.config['TD3']['load_second_agent'] == True:
            ...
            self.isAgent_discrete = config['TD3']['discrete'] if 'TD3' in config.keys() else None
    ...
    def mz_experiment2(self, participant_name):
            ...
            buffer_directory = os.path.join('results', self.mode, 'buffer', self.participant_name)
            if not os.path.exists(buffer_directory):
                os.makedirs(buffer_directory)
            
            # Determine unique filename for the buffer
            file_name = 'buffer.npy'
            c = 1
            full_path = os.path.join(buffer_directory, file_name)
            while os.path.isfile(full_path):
                file_name = f'buffer_{c}.npy'
                full_path = os.path.join(buffer_directory, file_name)
                c += 1
        
        # Save the buffer
            self.agent.memory.save_buffer(full_path)        
    ...
    def save_experience(self, interaction,block_number):
        ...
        """
        if self.config['Experiment']['agent'] == 'TD3':
    ...
    def get_agent_action(self, prev_observation, enemy_status=None,game_mode='train'):
        ...
        if not self.second_human and enemy_status == 'agent':
            ...
            agent_action = None
        #print(f"Env agent action: {env_agent_action}, Real agent action: {agent_action}")
    ...
    def compute_agent_action(self, observation,status):
        ...
        if status == 'agent':
            ...
            #print(f"Actor generated action: {agent_action}") 
            return agent_action
    ...
    def no_use(self, observation,status):
        raw_action = self.agent.actor.forward(observation)  # Use the actor's forward method
        discrete_action = int(np.clip(np.round(raw_action), -1, 1))  # Clamp to [-1, 0, 1]
        #print(f"Raw actor action: {raw_action}, Discrete action: {discrete_action}")
    ...
    #def compute_agent_action(self, observation, status):
        ...
        #discrete_action, argmax_action = self.agent.actor.sample_act(observation, for_training=False)
        #print(f"Discrete action sent to environment: {discrete_action}")
        #return discrete_action, argmax_action
    ...
    def grad_updates(self, update_cycles, block_number):
        ...
        if not self.second_human:
            ...
            for cycle_i in tqdm(range(update_cycles),file=sys.stdout):
                    if self.config['TD3']['freeze_agent'] == False:
                        policy_loss,q1_loss,q2_loss = self.agent.learn(block_number)
                        if cycle_i % 100 == 0:
                            print('Cycle:',cycle_i,'Policy Loss:',policy_loss,'Q1 Loss:',q1_loss,'Q2 Loss:',q2_loss)
                        
                    if self.mode == 'no_tl_two_agents':
                        if self.config['TD3']['freeze_second_agent'] == False:
